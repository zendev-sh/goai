...
GoAI is a Go SDK that provides one unified API across 24+ LLM providers. This document describes the overall architecture, key layers, data flow, and design decisions.
...
HTTP error response
  ...
  │   ├─ IsOverflow(message) — matches 14 regex patterns across 24+ providers
...
- **Cross‑provider error pattern matching**: 14 regex patterns detect "context overflow" across 24+ providers' inconsistent error messages, enabling uniform `ContextOverflowError`.
