...
func TestChat_Stream(t *testing.T) {
	...
	model := Chat("llama-3.3-70b", WithAPIKey("test-key"), WithBaseURL(server.URL))
...
func TestChat_Generate(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = fmt.Fprint(w, `{"id":"chatcmpl-123","model":"llama-3.3-70b","choices":[{"message":{"role":"assistant","content":"Hello world"},"finish_reason":"stop"}],"usage":{"prompt_tokens":10,"completion_tokens":5}}`)
	}))
	defer server.Close()
...
func TestModelID(t *testing.T) {
	model := Chat("llama-3.3-70b", WithAPIKey("k"))
	if model.ModelID() != "llama-3.3-70b" {
...
func TestChat_PromptCachingIgnored(t *testing.T) {
	...
	genServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = fmt.Fprint(w, `{"id":"chatcmpl-test","model":"llama-3.3-70b","choices":[{"index":0,"message":{"role":"assistant","content":"ok"},"finish_reason":"stop"}],"usage":{"prompt_tokens":5,"completion_tokens":2,"total_tokens":7}}`)
	}))
	defer genServer.Close()

	genModel := Chat("llama-3.3-70b", WithAPIKey("test-key"), WithBaseURL(genServer.URL))
	...
	streamModel := Chat("llama-3.3-70b", WithAPIKey("test-key"), WithBaseURL(streamServer.URL))
